...
import { ReportArrProps , Report } from '../../types/reportInterface'
import { deleteReport } from '../../services/reportApi';
import ViewBookReport from '../modals/ViewBookReport';
...
const ReportData: React.FC<ReportArrProps> = ({ data }) => {
  ...
  // Render Report based on condition
  const renderReport = (report: Report) => {
    switch (report.report_name) {
      case 'Books':
        return <ViewBookReport date={report.report_date} />;
      case 'Requests':
      case 'Inquiries':
      case 'News':
      default:
        return <Button variant="outlined" color="primary" sx={{ padding: 0,textTransform: 'none', height: '30px'}} >View</Button>;
    }
  };
